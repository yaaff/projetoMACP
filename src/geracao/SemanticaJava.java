...
import analise.VariavelDeclarada;
...
/**
 * Realiza a Analise Semantica e Gera��o de C�digo *
 */

public class SemanticaJava {
	/** Identificador de retorno de erro da An�lise Sintatica */
	public String erroSintatico = "";
	/** armazena o codigo gerado apos efetuar a analise lexica, sintatica e semantica */
	...
	/** Realiza a An�lise Semantica e geracao de codigo */
	public SemanticaJava( Token tk ) {
	...
	private String prog(){
		...
		if( prototipo != "" ){
			StringTokenizer aux = new StringTokenizer( prototipo ); //Prototipo da função - necessário em C/C++
	...
	private String dec(){
		...
		String err = "";
		codigoGerado += "import java.util.Scanner;\n\n"
                        + "public class Main{\n";
	...
	private String tipo_var(){
		...
		if( tipoBasico > 1 && tipoBasico < 7 ){// gambi
			...
			while ( aux.hasMoreTokens()){
				...
				if( tabelaHash.containsKey(rep) )
					return "Vari�vel \"" + rep + "\" declarada mais de uma vez.";
		...
		if( tipoBasico == 83 ){
			...
			while ( aux.hasMoreTokens()){
				String rep = aux.nextToken(",");
				if( tabelaHash.containsKey(rep) )
					return "M�dulo \"" + rep + "\" declarado mais de uma vez.";
		...
		if( tk.getClas() == 18 ){// consome o terminal "vetor"			
			...
			return err = erro("Esperando o tipo da vari�vel.");
		...
		if( tk.getClas() == 21 ){// verifica se o terminal � um n�mero
			...
			return err = erro("Argumento \"" + tk.getRepr() + "\" inv�lido.");
		...
		if ( inicio >= fim )
			return err = erro("Limite inferior n�o pode ser maior ou \n\t" +
	...
	private int tipo( ){
		int tip = tk.getClas();
		//verifica��o dos tipo de variavel declarada e convers�o para codigo java
		if( tip > 1 && tip < 7 ){
			switch( tip ){
				case 2:{
					codigoGerado += "\tpublic static int ";
					break;
				}
				case 3:{
					codigoGerado += "\tpublic static float ";
					break;
				}
				case 4:{
					codigoGerado += "\tpublic static bool ";
					break;
				}				
				case 5:{
					codigoGerado += "\tpublic static String ";
					break;
				}				
				case 6:{
					codigoGerado += "\tpublic static char ";
					break;
				}
				
			}			
			tk = tk.getNext(); //pega a declaração da variável
	...
	private String cmd(){
			...
			if(tk.getClas() == 20 ){  // verifica ocorrencia de um identificador
				if( ( tabelaHash.containsKey(tk.getRepr())) ){// verifica se foi declarado
					...
					if(!aux.isBasico() || aux.getTipoBasico() != 2 ){
						return err = erro("Vari�vel \"" + tk.getRepr() + "\" n�o � de tipo valido");
					}
				}
				else
					return err = erro("Vari�vel \"" + tk.getRepr() + "\" n�o declarada");
				...
				return err = erro("Esperando uma vari�vel");
		...
		if( tk.getClas() == 11){ // verifica a ocorr�ncia do comando "se"
			...
			if ( tk.getClas() == 12 ){ // verifica a ocorr�ncia do terminal "entao"
				...
				return err = erro("Esperando termo \"ent�o\".");
		...
		if ( tk.getClas() != 8 && tk.getClas() != 17 && tk.getClas() != 24 && tk.getClas() != 51)
			return err = erro("\"" + tk.getRepr() + "\" n�o representa comando v�lido.");
	...
	private String lista2(){
		...
		if( tk.getClas() == 20 ){// verifica se a variavel � representada por um identificador
			...
			codigoGerado += tk.getRepr();
                        ...
                        codigoGerado += " isReal? "+tk.isNumeroReal();
                        int tip = tk.isNumeroReal();
                        
                        //verifica��o dos tipo de variavel declarada e convers�o para codigo java
                        if( tip > 1 && tip < 7 ){
                                switch( tip ){
                                        case 2:{
                                                codigoGerado += " = entrada.nextInt(";
                                                break;
                                        }
                                        case 3:{
                                                codigoGerado += " = entrada.nextFloat(";
                                                break;
                                        }
                                        case 4:{
                                                codigoGerado += " = entrada.nextBoolean(";
                                                break;
                                        }
                                        case 5:{
                                                codigoGerado += " = entrada.next(";
                                                break;
                                        }
                                        case 6:{
                                                codigoGerado += " = entrada.nextChar(";
                                                break;
                                        }
	...
	private String lista3(){
		...
		if( tk.getClas() == 20 ){// verifica se a variavel � representada por um identificador
			...
			if ( tk.getClas() == 29 ){ // verifica se o terminal � um 'literal'
				...
				return err = erro("Esperando Vari�vel ou Literal de Cadeia");
	...
	private String list_var3(){
		...
		if( tk.getClas() == 27 ){ // consome o terminal "["
			...
			if( variavel.getTipo() == "VETOR" )
				return erro("Elemento de vetor \"" + variavel.getRepr() + " \" n�o indexado." );
	...
	private String indice(){
		...
		if ( tk.getClas() == 21 ){//numero
			...
			if( num < variavel.getIndexBegin() || num > variavel.getIndexEnd() )
				return err = erro( "�ndice de vetor inv�lido." );
	...
	private String expGeral(){
		...
		err = expArit();
		if ( err != "" )
			return err = erro("Express�o l�gica incorreta");
	...
	private String id(){
		...
		if ( tk.getClas() == 20 ){
			...
			err = erro("Identificador \"" + tk.getRepr() + "\" inv�lido.");
	...
	private String expLogicGeral(){
		...
		if(tk.getClas() == 20 && tabelaHash.containsKey(tk.getRepr()) ){
			...
			if( tk.getClas() == 21 || tk.getClas() == 30 ){
				...
				err = erro( "Express�o l�gica inv�lida." );
		}
	...
	private String outraParte(){
		...
		if( tk.getClas() == 20 && tabelaHash.containsKey(tk.getRepr()) ){
			...
			if( variavel.getTipoBasico() == 4 ){
				...
				return err = erro( "Variavel " + tk.getRepr() + " n�o � de tipo v�lido." );
			
		}
		
		err = termoLogico();
		if( err != "")
			return erro("Express�o l�gica inv�lida");
		...
		if ( tk.getClas() == 83 ){ // m�dulo
			...
			if( tk.getClas() == 20 ){// identifica��o do m�dulo
				...
				if( variavel.getTipoBasico() == 83 ){
					...
					return err = erro( "M�dulo funcional n�o declarado." );
			}
			else
				return err = erro("Esperando um identificador para o m�dulo funcional.");
